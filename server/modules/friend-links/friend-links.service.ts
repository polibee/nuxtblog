...
import { friendLinkChecks, friendLinkSubmissions, friendLinks } from '../../repositories/schema/friend-links'
...
export async function createSubmission(
  ...
  return { id: row!.insertId, siteStatus: site.status, backlinkStatus: backlink.status }
}
...
export async function getSubmission(id: number): Promise<Record<string, unknown>> {
  const [row] = await getDb().select().from(friendLinkSubmissions).where(eq(friendLinkSubmissions.id, id)).limit(1)
...
export async function createFriendLink(input: FriendLinkInput): Promise<number> {
  ...
  const [row] = await getDb().insert(friendLinks).values({
...
export async function updateFriendLink(id: number, input: Partial<FriendLinkInput>): Promise<void> {
  ...
  await getDb().update(friendLinks).set(patch).where(eq(friendLinks.id, id))
  await invalidateFriendLinksCache()
}

export async function deleteFriendLink(id: number): Promise<void> {
  await getDb().update(friendLinks).set({ status: 'removed', deletedAt: new Date() }).where(eq(friendLinks.id, id))
  await invalidateFriendLinksCache()
}
...
async function recordCheck(friendLinkId: number, checkType: 'site' | 'backlink', status: string, httpStatus: number | null, foundUrl: string | null): Promise<void> {
  await getDb().insert(friendLinkChecks).values({ friendLinkId, checkType, status, httpStatus, foundUrl })
}

export async function checkFriendLinkNow(id: number): Promise<{ siteStatus: string, backlinkStatus: string }> {
  const [link] = await getDb().select().from(friendLinks).where(eq(friendLinks.id, id)).limit(1)
  ...
  await getDb().update(friendLinks).set({
    backlinkStatus,
...
export async function runScheduledChecks(): Promise<{ checked: number, missing: number }> {
  if (!isBlogDbReady()) return { checked: 0, missing: 0 }
