...
export async function saveProfileBundle(bundle: ProfileBundle): Promise<void> {
  ...
  await db.transaction(async (tx) => {
    ...
    await tx.insert(authorProfileTranslations).values({
      profileId, localeId: defaultLocale.id, displayName: String(bundle.profile.displayName ?? ''), headline: String(bundle.profile.headline ?? ''), bio: bundle.profile.bio ? String(bundle.profile.bio) : null, location: String(bundle.profile.location ?? '')
