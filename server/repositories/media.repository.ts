...
async function findReferencedMediaIds(): Promise<number[]> {
  const { postTranslations } = await import('./schema/posts')
  const { sliderItems } = await import('./schema/slider')
  const { adCreativeTranslations } = await import('./schema/advertising')
  const { sidebarCards } = await import('./schema/sidebar-cards')
  const db = getDb()
...
export async function findMediaRow(id: number): Promise<typeof media.$inferSelect | undefined> {
  const rows = await getDb().select().from(media).where(eq(media.id, id)).limit(1)
  return rows[0]
}

export async function findMediaByStorageKey(key: string): Promise<typeof media.$inferSelect | undefined> {
  const rows = await getDb().select().from(media).where(eq(media.storageKey, key)).limit(1)
  return rows[0]
}
...
export async function insertMediaVariants(rows: MediaVariantRow[]): Promise<void> {
  ...
  await getDb().insert(mediaVariants).values(rows)
}
...
export async function listVariantsForMedia(mediaIds: number[]): Promise<Map<number, (MediaVariantRow & { url: string })[]>> {
  ...
  if (mediaIds.length === 0) return map
  const rows = await getDb().select().from(mediaVariants).where(inArray(mediaVariants.mediaId, mediaIds))
  for (const row of rows) {
...
export async function findVariantByStorageKey(key: string): Promise<typeof mediaVariants.$inferSelect | undefined> {
  const rows = await getDb().select().from(mediaVariants).where(eq(mediaVariants.storageKey, key)).limit(1)
  return rows[0]
}
