...
import { pageTranslations, pages } from './schema/pages'
...
export async function findPageRow(id: number): Promise<typeof pages.$inferSelect | undefined> {
  const rows = await getDb().select().from(pages).where(and(eq(pages.id, id), isNull(pages.deletedAt))).limit(1)
  return rows[0]
}
...
export async function insertPage(entity: {
  ...
  return getDb().transaction(async (tx) => {
    const [row] = await tx.insert(pages).values(entity)
    if (!row) throw new Error('page insert returned no id')
    if (translations.length > 0) {
      await tx.insert(pageTranslations).values(translations.map(t => ({ ...t, pageId: row.insertId })))
    }
    return row.insertId
  })
}
