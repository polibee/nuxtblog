...
import { postTranslations, posts } from './schema/posts'
...
export async function findPostRow(id: number): Promise<typeof posts.$inferSelect | undefined> {
  const rows = await getDb().select().from(posts).where(and(eq(posts.id, id), isNull(posts.deletedAt))).limit(1)
  return rows[0]
}
...
export async function getPost(id: number): Promise<PostRecord | undefined> {
  ...
  const [translationMap, localeRows] = await Promise.all([
    ...
    getDb().select({ id: locales.id, code: locales.code }).from(locales)
  ])
  return {
    id: row.id,
    alias: row.alias,
...
export async function insertPost(entity: {
  ...
  return getDb().transaction(async (tx) => {
    const [row] = await tx.insert(posts).values(entity)
    if (!row) throw new Error('post insert returned no id')
    if (translations.length > 0) {
      await tx.insert(postTranslations).values(translations.map(t => ({ ...t, postId: row.insertId })))
    }
    return row.insertId
  })
}
